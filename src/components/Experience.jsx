            ...
            {experience.map((job, i) => (
              <div
                ...
                <div className="hidden md:block w-[120px] shrink-0 text-right pr-6 pt-0.5">
                  <span className="font-sans text-[15px] text-text-muted">{job.period}</span>
                </div>
                ...
                <div className="flex-1 pl-4 border-l-2 border-accent-light md:border-0 md:pl-6">
                  <p className="md:hidden font-sans text-[15px] text-text-muted mb-1">{job.period}</p>
                  <p className="font-sans text-base font-semibold text-text-base leading-tight mb-0">
                    {job.company}
                  </p>
                  <p className="font-sans text-[16px] text-text-muted mt-0.5" style={{ marginBottom: '10px' }}>
                    {job.role}{job.location ? ` · ${job.location}` : ''}
                  </p>
                  <div className="space-y-3">
                    {job.highlights.slice(0, 2).map((bullet, j) => (
                      <p key={j} className="font-sans text-[16px] text-text-muted mb-0"
